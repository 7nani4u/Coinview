# -*- coding: utf-8 -*-
# SignalCoin - main.py (Vercel 공식 방식)
#
# Vercel 공식 FastAPI 예제 구조:
#   - 파일 위치: 프로젝트 루트의 main.py
#   - vercel.json 없이도 자동 감지됨
#   - HTML은 HTMLResponse로 직접 서빙 (StaticFiles 사용 금지)
#   - 모든 API 경로는 /api/* 로 시작
...
<aside id="sidebar">
  ...
  <div class="sidebar-section">
    ...
    <label>🛑 손절 배수 (σ): <strong id="sl-label">2.0</strong></label>
...
function setupIntervalInfo() {
  const infoMap = {
    "1m": "⏱️ 1분봉: 최근 7일만 지원 (초단타 매매용)",
    "5m": "⏱️ 5분봉: 최근 60일만 지원 (단타 매매용)",
    "1h": "⏱️ 1시간봉: 최근 2년만 지원 (스윙 트레이딩용)",
...
async function loadFearGreed() {
  try {
    ...
    if (data.value < 25) {
      ...
      msgEl.textContent = "↗️ 극도의 공포 → 매수 기회";
    } else if (data.value > 75) {
      msgEl.className = "warn-box";
      msgEl.textContent = "↘️ 극도의 탐욕 → 매도 고려";
    } else {
      msgEl.className = "info-box";
      msgEl.textContent = "🟡 중립 상태 → 추세 관찰";
    }
  } catch {
    document.getElementById("fg-message").textContent = "⚠️ 시장 심리 데이터를 불러올 수 없습니다.";
  }
}
...
function renderSummaryMetrics(data) {
  ...
  const cards = [
    ...
    { label: "RSI (14)", value: rsi.toFixed(1), delta: rsi > 70 ? "과매수" : rsi < 30 ? "과매도" : "중립", deltaClass: rsi > 70 ? "delta-down" : rsi < 30 ? "delta-up" : "", emoji: "📊" },
  ...
  document.getElementById("signal-content").innerHTML = `
    <div style="display:flex; gap:24px; flex-wrap:wrap; align-items:flex-start;">
      ...
      <div style="flex:1; min-width:160px;">
        ...
        <table>
          <tr><td>RSI 점수</td><td><strong>${signal.rsi_score.toFixed(1)}</strong></td></tr>
          <tr><td>추세 점수</td><td><strong>${signal.trend_score.toFixed(1)}</strong></td></tr>
        </table>
      </div>
    </div>
  `;
...
function renderIndicatorsTable(ind, price) {
  const rows = [
    ...
    ["RSI (14)", ind.rsi ? ind.rsi.toFixed(2) : "N/A", ind.rsi > 70 ? "⚠️ 과매수" : ind.rsi < 30 ? "✅ 과매도 (매수 기회)" : "중립"],
    ...
    ["볼린저 상단", ind.bb_upper ? `$${ind.bb_upper.toLocaleString(undefined, {maximumFractionDigits:4})}` : "N/A", price > ind.bb_upper ? "⚠️ 상단 돌파" : ""],
  ...
  document.getElementById("indicators-content").innerHTML = `
    <table>
      ...
      <tbody>${rows.map(r => `<tr><td>${r[0]}</td><td><strong>${r[1]}</strong></td><td style="color:#555;">${r[2]}</td></tr>`).join("")}</tbody>
    </table>
  `;
}
...
function renderCharts(chartData, ticker) {
  ...
  }], { ...layout_base, title: `${ticker} 캔들스틱 차트` }, { responsive: true });
  ...
  }], { ...layout_base, title: "거래량" }, { responsive: true });
  ...
  }], { ...layout_base, title: "MACD" }, { responsive: true });
  ...
  }], { ...layout_base, title: "볼린저 밴드" }, { responsive: true });
}
