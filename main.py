...
def _fetch_ohlcv(ticker: str, days: int, interval: str = "1d") -> pd.DataFrame:
    """원본 load_crypto_data (2211행) 경량 버전"""
    end = datetime.now()
...
def _calc_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """원본 calculate_indicators_wilders (2433행) 핵심 지표 경량 버전"""
    close = df["Close"]
...
def _calc_signal_score(df: pd.DataFrame) -> dict:
    """원본 calculate_signal_score (430행) 이식"""
...
def _calc_backtest(df: pd.DataFrame) -> dict:
    """원본 backtest_portfolio_simple (1910행) 이식"""
...
def _calc_trading_metrics(ticker: str) -> dict:
    """원본 calculate_trading_metrics (502행) 이식"""
...
def root():
    ...
    <head>
        ...
        <style>
            body { font-family: -apple-system, sans-serif; background: #0a0a0a; color: #e0e0e0; max-width: 800px; margin: 60px auto; padding: 0 20px; }
            h1 { color: #00d4aa; }
            h2 { color: #888; font-size: 0.9rem; font-weight: normal; margin-top: -10px; }
            .card { background: #1a1a1a; border: 1px solid #333; border-radius: 8px; padding: 16px 20px; margin: 10px 0; }
            .method { background: #00d4aa; color: #000; padding: 2px 8px; border-radius: 4px; font-size: 0.8rem; font-weight: bold; margin-right: 8px; }
    ...
    <body>
        <h1>📊 SignalCoin API v3.0</h1>
        <h2>암호화폐 기술적 분석 API (Vercel Serverless Edition)</h2>

        <div class="card">
            <span class="method">GET</span> <a href="/api/v2/market/fear-and-greed">/api/v2/market/fear-and-greed</a>
            <p>공포-탐욕 지수 조회</p>
        </div>
        <div class="card">
            <span class="method">GET</span> <a href="/api/v2/coins/list">/api/v2/coins/list</a>
            <p>CoinGecko 시가총액 상위 코인 목록 조회</p>
        </div>
        <div class="card">
            <span class="method">GET</span> <code>/api/v2/market/price/{ticker}</code>
            <p>OHLCV 가격 데이터 조회 · 예: <a href="/api/v2/market/price/BTC-USD">/api/v2/market/price/BTC-USD</a></p>
        </div>
        <div class="card">
            <span class="method">GET</span> <code>/api/v2/analysis/indicators/{ticker}</code>
            <p>RSI, MACD, 볼린저밴드 등 기술적 지표 · 예: <a href="/api/v2/analysis/indicators/BTC-USD">/api/v2/analysis/indicators/BTC-USD</a></p>
        </div>
        <div class="card">
            <span class="method">GET</span> <code>/api/v2/analysis/signal/{ticker}</code>
            <p>매수/매도 신호 점수 · 예: <a href="/api/v2/analysis/signal/BTC-USD">/api/v2/analysis/signal/BTC-USD</a></p>
        </div>
        <div class="card">
            <span class="method">GET</span> <code>/api/v2/analysis/metrics/{ticker}</code>
            <p>기간별 수익률 및 시장 심리 · 예: <a href="/api/v2/analysis/metrics/BTC-USD">/api/v2/analysis/metrics/BTC-USD</a></p>
        </div>
        <div class="card">
            <span class="method">GET</span> <code>/api/v2/portfolio/backtest/{ticker}</code>
            <p>Buy-and-Hold 백테스트 · 예: <a href="/api/v2/portfolio/backtest/BTC-USD">/api/v2/portfolio/backtest/BTC-USD</a></p>
        </div>
        <div class="card">
            <span class="method">GET</span> <a href="/docs">/docs</a> &nbsp;|&nbsp; <a href="/redoc">/redoc</a>
...
@app.get("/api/v2/market/fear-and-greed", response_model=FearGreedResponse, tags=["Market Data"])
...
@app.get("/api/v2/coins/list", response_model=List[CoinInfo], tags=["Market Data"])
...
@app.get("/api/v2/analysis/indicators/{ticker}", response_model=IndicatorResponse, tags=["Technical Analysis"])
...
@app.get("/api/v2/analysis/signal/{ticker}", response_model=SignalResponse, tags=["Technical Analysis"])
def get_signal(ticker: str, days: int = Query(120, ge=50, le=730)):
    """RSI + EMA 기반 매수/매도 신호 점수 반환 (원본 calculate_signal_score 이식)"""
    df = _fetch_ohlcv(ticker, days, "1d")
    if len(df) < 50:
...
@app.get("/api/v2/analysis/metrics/{ticker}", response_model=TradingMetricsResponse, tags=["Technical Analysis"])
def get_trading_metrics(ticker: str):
    """기간별 수익률 및 시장 심리 지표 반환 (원본 calculate_trading_metrics 이식)"""
    metrics = _calc_trading_metrics(ticker)
    return TradingMetricsResponse(ticker=ticker, **metrics)


@app.get("/api/v2/portfolio/backtest/{ticker}", response_model=BacktestResponse, tags=["Portfolio"])
def run_backtest(ticker: str, days: int = Query(365, ge=30, le=1825)):
    """Buy-and-Hold 백테스트 (원본 backtest_portfolio_simple 이식)"""
    df = _fetch_ohlcv(ticker, days, "1d")
