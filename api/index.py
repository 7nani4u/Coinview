# -*- coding: utf-8 -*-
"""
CryptoOracle - StockOracle 기반 암호화폐 AI 레버리지 예측 통합 플랫폼
===================================================
[구조 설명]
- StockOracle(주식 분석)을 암호화폐 전용으로 완전 변환
- ⚡ 레버리지 예측 엔진을 🔮 예측 탭의 핵심 기능으로 통합
  (방향성 확률 · 진입/손절 구간 · 1~3차 목표가 · R/R · 신뢰도 · 청산 위험도 · 추세 강도 · AI 종합 의견)
...
# =============================================================================
# ── 통합 예측 빌더 (⚡ 레버리지 엔진 → 🔮 예측 탭 핵심 통합)
#    방향성 확률 · 진입/손절 구간 · 1~3차 목표가 · 리스크(청산/추세/R:R/신뢰도) · AI 의견
# =============================================================================

def calc_direction_probability(score, rsi, macd, macd_signal, ema12, ema26,
                               bb_upper, bb_lower, price, funding_rate):
    """기술적 종합점수 + 모멘텀 확인 신호를 결합해 상승/하락 가능성(%) 산출."""
    raw = float(score)  # 0~100 (50 중립)
    if ema12 and ema26:
        raw += 4 if ema12 > ema26 else -4          # 추세(EMA 배열) 확인
    raw += 3 if macd > macd_signal else -3          # 모멘텀(MACD) 확인
    if bb_upper and price >= bb_upper:              # 볼린저 과열 → 반전 위험
        raw -= 3
    elif bb_lower and price <= bb_lower:            # 볼린저 침체 → 반등 기대
        raw += 3
    if rsi >= 75:                                   # RSI 극단 보정
        raw -= 3
    elif rsi <= 25:
        raw += 3
    if funding_rate is not None:                    # 펀딩비 쏠림 → 반대매매 압력
        if funding_rate > 0.05:
            raw -= 2
        elif funding_rate < -0.05:
            raw += 2
    prob_up = max(5.0, min(95.0, raw))
    return round(prob_up, 1), round(100.0 - prob_up, 1)


def calc_liquidation_risk(price, leverage, position, atr_pct, mmr=0.005):
    """레버리지·유지증거금 기준 청산가 및 청산 위험도 산출 (격리 마진 가정)."""
    lev = max(1, int(leverage or 1))
    if not price or price <= 0:
        return None
    if position == "Short":
        liq = price * (1 + 1.0 / lev - mmr)
    else:  # Long(기본)
        liq = price * (1 - 1.0 / lev + mmr)
    distance_pct = abs(liq - price) / price * 100
    # 청산까지 거리를 ATR(%) 배수로 환산 → 변동성 대비 안전 마진
    atr_buffer = (distance_pct / atr_pct) if atr_pct and atr_pct > 0 else 99
    if atr_buffer <= 1.5:
        level, color, desc = "매우 높음", "#ff0000", "변동성 1.5 ATR 내 청산 — 즉시 레버리지 축소"
    elif atr_buffer <= 3:
        level, color, desc = "높음", "#f85149", "변동성 3 ATR 내 청산 가능 — 손절 필수"
    elif atr_buffer <= 6:
        level, color, desc = "보통", "#d29922", "일반적 안전 마진 — 손절 준수 시 관리 가능"
    else:
        level, color, desc = "낮음", "#3fb950", "청산가까지 충분한 거리 확보"
    return {
        "liquidation_price": round(liq, 6),
        "distance_pct": round(distance_pct, 2),
        "atr_buffer": round(atr_buffer, 1),
        "level": level, "color": color, "desc": desc,
    }


def calc_trend_strength(score, ema12, ema26, rsi):
    """추세 강도(0~100) 및 등급 산출."""
    strength = abs(score - 50) * 1.6                # 종합점수의 중립 이탈도
    if ema12 and ema26 and ema26 != 0:
        spread = abs(ema12 - ema26) / ema26 * 100
        strength += min(spread * 4, 25)
    if rsi >= 60 or rsi <= 40:
        strength += 8
    strength = max(0, min(100, strength))
    if strength >= 70:   grade = "매우 강함"
    elif strength >= 45: grade = "강함"
    elif strength >= 25: grade = "보통"
    else:                grade = "약함 (횡보)"
    return round(strength, 0), grade


def build_prediction(price, atr, atr_pct, score, leverage_info, dd, funding_rate):
    """🔮 예측 탭 통합 데이터: 방향성·진입·손절·목표·리스크·AI의견."""
    def v(k):
        a = dd.get(k, [])
        return float(a[-1]) if a else 0.0
    rsi = v("RSI"); macd = v("MACD"); macd_sig = v("Signal_Line")
    ema12 = v("EMA12"); ema26 = v("EMA26")
    bb_u = v("BB_Upper"); bb_l = v("BB_Lower")
    vol_30d = v("Volatility_30d")

    prob_up, prob_down = calc_direction_probability(
        score, rsi, macd, macd_sig, ema12, ema26, bb_u, bb_l, price, funding_rate)

    # 방향 결정: 확률 우위 (격차 8%p 미만이면 관망)
    if prob_up - prob_down >= 8:
        direction = "LONG"
    elif prob_down - prob_up >= 8:
        direction = "SHORT"
    else:
        direction = "NEUTRAL"
    ...
    # 진입 구간 / 손절 / 1~3차 목표가 (방향별)
    # 손절 1.5 ATR 고정, 목표가 사다리 3.0/4.5/6.0 ATR → 1차 R:R 1:2 (엔진 설계 정합)
    if direction == "SHORT":
        entry_low, entry_high = price - atr * 0.2, price + atr * 0.5
        stop_loss = price + atr * 1.5
        targets = [round(price - atr * m, 6) for m in (3.0, 4.5, 6.0)]
    else:  # LONG / NEUTRAL(롱 관점 참고치)
        entry_low, entry_high = price - atr * 0.5, price + atr * 0.2
        stop_loss = price - atr * 1.5
        targets = [round(price + atr * m, 6) for m in (3.0, 4.5, 6.0)]
    target_pcts = [round((t - price) / price * 100, 2) for t in targets]

    # 손익비 R:R = 1차 목표 거리 / 손절 거리
    risk_dist = abs(price - stop_loss)
    reward_dist = abs(targets[0] - price)
    ...
    trend_val, trend_grade = calc_trend_strength(score, ema12, ema26, rsi)

    # 신뢰도: 방향 확률 우위 + 추세강도 + 위험등급 보정
    conf = abs(prob_up - prob_down) * 0.6 + trend_val * 0.4
    if leverage_info:
        conf += {"Low": 5, "Medium": 0, "High": -8, "Extreme": -15}.get(
            leverage_info.get("risk_grade", "Medium"), 0)
    ...
    if direction == "NEUTRAL":
        opinion = (f"상승 {prob_up:.0f}% / 하락 {prob_down:.0f}%로 방향성이 불분명합니다. "
                   f"추세 강도 '{trend_grade}', 변동성 위험 '{vol_risk[0]}' 상태로 "
                   f"뚜렷한 신호가 확인될 때까지 관망을 권장합니다.")
    else:
        opinion = (f"종합 분석 결과 {dir_kr} 우위(상승 {prob_up:.0f}% / 하락 {prob_down:.0f}%)입니다. "
                   f"추천 레버리지 {lev}x, 손익비 1:{rr_ratio}, 추세 강도 '{trend_grade}'. "
                   f"진입 후 손절가 {stop_loss:,.4f} 준수가 필수이며, "
                   f"청산 위험도는 '{(liq or {}).get('level', '-')}' 입니다.")
    ...
    return {
        ...
        "entry_zone": {"low": round(min(entry_low, entry_high), 6),
                       "high": round(max(entry_low, entry_high), 6)},
...
def route(path: str, params: Dict) -> Optional[Dict]:
    ...
    if path == "/api/coin":
        ...
        prediction = build_prediction(last, atr, atr_pct, score, leverage_info, dd, funding)
