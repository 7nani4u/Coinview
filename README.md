...
**CryptoOracle**는 StockOracle(주식 예측 시스템)의 구조·UI·예측 엔진을 기반으로, 백엔드를 암호화폐 시장으로 완전 전환하고 **⚡ 레버리지 예측 엔진을 🔮 예측 탭의 핵심 기능으로 통합**한 암호화폐 전용 예측 플랫폼입니다. Binance 공개 REST API로 가격·거래량·펀딩비·오픈인터레스트·기술지표를 분석합니다. 확률적 참고 정보를 제공하며, 특정 수익이나 방향을 보장하는 자동매매 시스템이 아닙니다.

## 🔮 예측 탭 (레버리지 통합) — 핵심 기능
하나의 `/api/coin` 호출이 다음 통합 예측(`prediction`)을 반환하고, 🔮 예측 탭이 7개 섹션으로 렌더링합니다.
1. **예측 결과** — 상승/하락 가능성(%) 게이지
2. **추천 포지션** — LONG / SHORT (상승확률>하락확률→LONG, 반대→SHORT, 격차 8%p 미만은 관망) + 추천 레버리지
3. **진입 구간** — 추천 매수/숏 진입 범위 (USDT)
4. **손절 가격** — ATR×1.5 기반
5. **목표 가격** — 1차/2차/3차 (ATR 3.0/4.5/6.0배)
6. **리스크 분석** — 예상 손익비(R/R) · 신뢰도 점수 · 변동성 위험도 · 청산 위험도(청산가·ATR 버퍼) · 추세 강도
7. **AI 종합 의견** — 방향·레버리지·손절·청산을 종합한 서술

데이터 산출: `calc_leverage_recommendation`(레버리지·방향) + `calc_direction_probability`(방향성 확률) + `calc_liquidation_risk`(청산) + `calc_trend_strength`(추세) + `build_prediction`(통합).
